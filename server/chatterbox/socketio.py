...
def message_event(data):
    ...
    with session_scope() as session:
        ...
        msg_id = msg.id
        response = {"message": MessageSchema().dump(msg), "status_code": 200}

    if not Message.query.filter_by(id=msg_id).one_or_none():
        response = {"message": "Failed to insert message.", "status_code": 403}
...
def unseen_messages(data):
    ...
    unseen_messages = {
        row[0]: {"count": row[1], "max_datetime": row[2].isoformat()}
